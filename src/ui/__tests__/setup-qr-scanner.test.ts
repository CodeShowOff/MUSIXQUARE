...
describe('guest setup QR scanner', () => {
  ...
  it('accepts any domain and emits one join code', async () => {
    const camera = fakeCamera();
    mocks.decodeQr
      .mockReturnValueOnce({ data: 'https://evil.example/musixquare.com/123456/extra' })
      .mockReturnValueOnce({ data: 'https://musixquare.com/123456/extra' })
      .mockReturnValueOnce({ data: 'MUSIXQUARE.COM/1234' })
      .mockReturnValueOnce({ data: 'LOCAL.APP:3000/654321' });
    const onCode = vi.fn();
    const onStartFromGesture = vi.fn();

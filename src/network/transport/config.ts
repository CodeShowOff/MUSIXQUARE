...
export function isLocalTransportHost(hostname = location.hostname): boolean {
  if (isLoopbackHostname(hostname as unknown)) return true;
  return /^(10\.|192\.168\.|172\.(1[6-9]|2[0-9]|3[0-1])\.)/.test(hostname) || (hostname as string).endsWith('.local');
}

// [TEMPORARY HACK]
// These are hardcoded to return undefined to force the app to fall back to PeerJS 
// for all signaling (even in production deployments). This prevents the app from 
// trying to connect to the official MUSIXQUARE signaling servers which would block 
// custom deployed domains via CORS.
// TODO: Revert these back to the original ternary logic when you deploy your own backend.

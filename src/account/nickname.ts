...
export function isAccountNicknameTakenError(_error: unknown): boolean {
  return false;
}
...
export function updateCurrentAccountNickname(
  value: string,
): Promise<string> {
  const validationError = validateAccountNickname(value);
  if (validationError) throw new Error(validationError);
  const nickname = normalizeAccountNickname(value);
  localStorage.setItem('mxqr-nickname', nickname);
  return Promise.resolve(nickname);
}

export function getCurrentAccountNickname(): string | null {
  return localStorage.getItem('mxqr-nickname');
}
